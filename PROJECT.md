...
- [2026-04-22] Devan — TCP Autonomous Ops Phase A shipped (review dashboard + Monday briefing)
  ...
  - Self-test: `npm run typecheck` clean. `npm run build` clean (`/review/[token]` and `/api/review/[token]` registered). `DRY_RUN=1 node scripts/brett-monday-briefing.mjs` renders without crashing on empty review_requests (subject="TCP: Week of Apr 22", shipped=4, pending=0). `/review/<valid-token-no-row>` returns 404 not 500. `/review/garbage` returns 404. `PATCH /api/review/garbage` returns 404. Token round-trip verified (sign + verify + tamper detection).
  - Bernard actions to enable Phase A:
    1. Run `supabase/migrations/002_create_review_requests.sql` in the Supabase SQL Editor (project `tpihpenmsiojzznpcmcr`).
    2. Set env vars in Vercel + `.env.local`:
       - `REVIEW_TOKEN_SECRET` — generate with `openssl rand -hex 32` (REQUIRED, fails closed without it).
       - `RESEND_TO_BRETT` — optional override, defaults to `brettlc2113@gmail.com`.
    3. Install crontab for 7am PT Monday: `0 7 * * 1 /Users/bcarter/Desktop/Claude\ Agents/projects/tiktok-creativity-program/scripts/brett-monday-briefing.sh` (timezone follows the host; cron is local time, so this fires 7am whatever the Mac is set to — confirm Mac is on America/Los_Angeles).
  - Status: READY FOR REVIEW
