...
// ── Niche RPM data ──────────────────────────────────────────────────────────
// Creativity Program (Rewards) RPM ranges by niche
const niches = [
...
// Old Creator Fund RPM (same for all niches)
const CREATOR_FUND_RPM_LOW = 0.02
const CREATOR_FUND_RPM_HIGH = 0.04
const QUALIFIED_VIEW_RATIO = 0.82

type Program = 'rewards' | 'fund' | 'both'
...
export function EarningsComparisonCalculator({ className }: { className?: string }) {
  ...
  const results = useMemo(() => {
    ...
    // Rewards program
    const rewardsLow = (qualifiedViews / 1000) * niche.rpmLow
    const rewardsHigh = (qualifiedViews / 1000) * niche.rpmHigh
    const rewardsMid = (rewardsLow + rewardsHigh) / 2
    const bonusMultiplier = includeBonus ? 1.2 : 1

    // Creator Fund
    const fundLow = (qualifiedViews / 1000) * CREATOR_FUND_RPM_LOW
    const fundHigh = (qualifiedViews / 1000) * CREATOR_FUND_RPM_HIGH
    const fundMid = (fundLow + fundHigh) / 2

    // Multiplier (how much more Rewards pays vs Fund)
    const multiplier = fundMid > 0 ? rewardsMid / fundMid : 0

    return {
      totalMonthlyViews,
      qualifiedViews,
      rewards: {
        low: rewardsLow * bonusMultiplier,
        high: rewardsHigh * bonusMultiplier,
        mid: rewardsMid * bonusMultiplier,
        yearlyLow: rewardsLow * bonusMultiplier * 12,
        yearlyHigh: rewardsHigh * bonusMultiplier * 12,
        yearlyMid: rewardsMid * bonusMultiplier * 12,
      },
      fund: {
        low: fundLow,
        high: fundHigh,
        mid: fundMid,
        yearlyLow: fundLow * 12,
        yearlyHigh: fundHigh * 12,
        yearlyMid: fundMid * 12,
      },
      multiplier,
  ...
  const handleInteraction = () => {
    trackCalculatorUsed('earnings_comparison_calculator', {
      viewsPerVideo,
      videosPerMonth,
      niche: nicheId,
      program,
  ...
  const showRewards = program === 'rewards' || program === 'both'
  const showFund = program === 'fund' || program === 'both'
        ...
        <div className="grid gap-0 lg:grid-cols-[1.2fr_1fr]">
          ...
          <div className="p-6 lg:p-8 space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1 rounded-xl bg-background-surface p-1">
                {([
                  { value: 'both', label: 'Compare Both' },
                  { value: 'rewards', label: 'Rewards Only' },
                  { value: 'fund', label: 'Creator Fund' },
                ] as const).map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => { setProgram(opt.value); handleInteraction() }}
                    className={cn(
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-text-muted">
                Rewards RPM for {niche.label}: {results.rpmRange} per 1,000 views
              </p>
            </div>
          ...
          <div className="border-t border-border-default bg-background-warm p-6 lg:border-t-0 lg:border-l lg:p-8">
            ...
            {showRewards && (
              <div className="mb-5">
                ...
                >
                  $<NumberTicker
                    value={results.rewards.low}
                    decimalPlaces={2}
                    className="text-green-600"
                  />
                  {' - $'}
                  <NumberTicker
                    value={results.rewards.high}
                    decimalPlaces={2}
                    className="text-green-600"
                  />
                </div>
                ...
                <p
                  ...
                  style={{ fontFamily: 'var(--font-mono)' }}
                >
                  {formatMoney(results.rewards.yearlyLow)} - {formatMoney(results.rewards.yearlyHigh)} / year
                </p>
              </div>
            )}
            ...
            {showFund && (
              <div className="mb-5">
                ...
                <div
                  className="text-[2.25rem] font-bold text-red-500 leading-none"
                  style={{ fontFamily: 'var(--font-mono)', fontWeight: 700 }}
                  aria-live="polite"
                  aria-atomic="true"
                >
                  $<NumberTicker
                    value={results.fund.low}
                    decimalPlaces={2}
                    className="text-red-500"
                  />
                  {' - $'}
                  <NumberTicker
                    value={results.fund.high}
                    decimalPlaces={2}
                    className="text-red-500"
                  />
                </div>
                ...
                <p
                  ...
                  style={{ fontFamily: 'var(--font-mono)' }}
                >
                  {formatMoney(results.fund.yearlyLow)} - {formatMoney(results.fund.yearlyHigh)} / year
                </p>
              </div>
            )}
            ...
            {program === 'both' && (
              <div className="rounded-xl bg-green-50 border border-green-200 p-4 mt-4">
                <p className="text-sm font-bold text-green-800">
                  Creator Rewards pays{' '}
                  <span
                    className="text-lg"
                    style={{ fontFamily: 'var(--font-mono)' }}
                  >
                    {results.multiplier.toFixed(0)}x
                  </span>{' '}
                  more than the old Creator Fund
                </p>
                <p className="text-xs text-green-700 mt-1">
                  That is{' '}
                  <span className="font-semibold" style={{ fontFamily: 'var(--font-mono)' }}>
                    {formatMoney(results.rewards.mid - results.fund.mid)}
                  </span>{' '}
                  more per month with the same content.
                </p>
              </div>
            )}
            ...
            <div className="mt-5 pt-4 border-t border-border-default space-y-2">
              ...
              {showRewards && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-text-muted">Rewards RPM range</span>
                  <span
                    className="font-semibold text-brand-ink"
                    style={{ fontFamily: 'var(--font-mono)' }}
                  >
              ...
              {showFund && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-text-muted">Creator Fund RPM</span>
                  <span
                    className="font-semibold text-brand-ink"
                    style={{ fontFamily: 'var(--font-mono)' }}
                  >
            ...
            <p className="mt-4 text-xs text-text-muted leading-[1.6]">
