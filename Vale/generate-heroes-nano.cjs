...
// Hardcoded key -- the GEMINI_API_KEY env var contains an expired key
const API_KEY = 'AIzaSyCltPluWUe42OykNuJVqhMRmxdoLCXyuQI';
...
async function generateImage(prompt) {
  ...
  var result = await httpsPost(postData);
  ...
  for (var ci = 0; ci < candidates.length; ci++) {
    ...
    for (var pi = 0; pi < parts.length; pi++) {
      if (parts[pi].inlineData && parts[pi].inlineData.data) {
        return {
          base64: parts[pi].inlineData.data,
          mimeType: parts[pi].inlineData.mimeType || 'image/jpeg'
        };
      }
    }
  }
...
async function saveHeroAndThumb(slug, base64Data) {
  ...
  var heroPath = path.join(OUTPUT_DIR, 'hero-' + slug + '.webp');
  var thumbPath = path.join(OUTPUT_DIR, 'thumb-' + slug + '.webp');
  ...
  return { heroPath: heroPath, thumbPath: thumbPath };
}
...
async function run() {
  ...
  for (var i = 0; i < GUIDES.length; i++) {
    var slug = GUIDES[i].slug;
    var scene = GUIDES[i].scene;
    var heroPath = path.join(OUTPUT_DIR, 'hero-' + slug + '.webp');
    ...
    while (retries <= maxRetries && !succeeded) {
      try {
        var imgResult = await generateImage(prompt);
        await saveHeroAndThumb(slug, imgResult.base64);
        ...
        if (err.message.indexOf('429') !== -1 || err.message.toLowerCase().indexOf('quota') !== -1 || err.message.toLowerCase().indexOf('resource_exhausted') !== -1) {
        ...
        if (retries > maxRetries) {
          results.failed.push({ slug: slug, error: err.message });
        }
      }
    }
  }
  ...
  if (results.failed.length > 0) {
    console.log('\nFailed slugs:');
    results.failed.forEach(function(f) { console.log('  - ' + f.slug + ': ' + f.error); });
  }
